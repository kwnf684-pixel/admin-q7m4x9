...
watchSession();
window.addEventListener('storage',e=>{if(e.key===key){try{session=e.newValue?JSON.parse(e.newValue):null;checked=false;watchSession();emit();}catch{session=null;emit();}}});
