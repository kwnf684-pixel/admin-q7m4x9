...
export async function changeLocalPassword(current:string,next:string){if(!client||!session)throw Error('سجّل الدخول أولًا.');await client.action(ref<'action'>('auth:changeAdminPassword'),{token:session.token,current,password:next});return true;}
