...
export async function session(ctx:QueryCtx,token:string,admin=false){
 ...
 if(s.role==='admin'){const credential=await ctx.db.query('adminCredentials').unique();if(credential&&credential.passwordHash!==s.credentialHash)throw new ConvexError('UNAUTHORIZED');}
...
export const issue=internalMutation({args:{tokenHash:v.string(),role:v.union(v.literal('admin'),v.literal('merchant')),merchantId:v.optional(v.id('merchants')),expectedHash:v.optional(v.string())},handler:async(ctx,a)=>{const m=a.merchantId?await ctx.db.get(a.merchantId):null;if(a.role==='merchant'&&(!m||m.passwordHash!==a.expectedHash||m.deleted||m.frozen))throw new ConvexError('ACCESS_REVOKED');if(a.role==='admin'){const credential=await ctx.db.query('adminCredentials').unique();if(a.expectedHash!==(credential?.passwordHash??process.env.ADMIN_PASSWORD_HASH))throw new ConvexError('UNAUTHORIZED');}const loginKey=a.role==='admin'?'admin':m!.whatsapp;const limit=await ctx.db.query('loginLimits').withIndex('key',q=>q.eq('key',loginKey)).unique();if(limit)await ctx.db.delete(limit._id);const expiresAt=Date.now()+7*86400000;await ctx.db.insert('sessions',{tokenHash:a.tokenHash,role:a.role,merchantId:a.merchantId,credentialHash:a.expectedHash??m?.passwordHash,expiresAt});return {role:a.role,merchantId:a.merchantId??null,name:m?.name??'الإدارة',expiresAt:m?.expiresAt??expiresAt,sessionExpiresAt:expiresAt};}});
export const status=query({args:{token:v.string()},handler:async(ctx,{token})=>{
 ...
 if(s.role==='admin'){const credential=await ctx.db.query('adminCredentials').unique();if(credential&&credential.passwordHash!==s.credentialHash)return {status:'unauthorized'};}
 const m=s.merchantId?await ctx.db.get(s.merchantId):null;
 if(s.role==='merchant'){
...
export const adminCredential=internalQuery({args:{},handler:async(ctx)=>{const row=await ctx.db.query('adminCredentials').unique();return {salt:row?.salt??process.env.ADMIN_PASSWORD_SALT,passwordHash:row?.passwordHash??process.env.ADMIN_PASSWORD_HASH};}});
export const checkAdmin=internalQuery({args:{token:v.string()},handler:async(ctx,{token})=>{await session(ctx,await digest(token),true);return true;}});
export const replaceAdminPassword=internalMutation({args:{token:v.string(),previous:v.string(),salt:v.string(),passwordHash:v.string()},handler:async(ctx,a)=>{const {s}=await session(ctx,await digest(a.token),true);const row=await ctx.db.query('adminCredentials').unique();if((row?.passwordHash??process.env.ADMIN_PASSWORD_HASH)!==a.previous)throw new ConvexError('UNAUTHORIZED');if(row)await ctx.db.patch(row._id,{salt:a.salt,passwordHash:a.passwordHash});else await ctx.db.insert('adminCredentials',{salt:a.salt,passwordHash:a.passwordHash});await ctx.db.patch(s._id,{credentialHash:a.passwordHash});const limit=await ctx.db.query('loginLimits').withIndex('key',q=>q.eq('key','change:admin')).unique();if(limit)await ctx.db.delete(limit._id);return {ok:true};}});
