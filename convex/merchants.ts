...
export const remove=mutation({args:{token:v.string(),id:v.id('merchants'),code:v.string()},handler:async(ctx,a)=>{await session(ctx,await digest(a.token),true);if(a.code.trim().replace(/[٠-٩]/g,c=>String('٠١٢٣٤٥٦٧٨٩'.indexOf(c))).replace(/[۰-۹]/g,c=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(c)))!=='101')throw new ConvexError('INVALID_DELETE_CODE');const m=await ctx.db.get(a.id);if(!m)return;if(!m.deleted)await ctx.db.patch(a.id,{deleted:true,frozen:true});await ctx.scheduler.runAfter(0,anyApi.merchants.purge,{id:a.id});}});
